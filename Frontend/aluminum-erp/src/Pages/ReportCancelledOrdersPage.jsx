...
import useReportCancelledOrdersStore from "../stores/ReportCancelledOrdersStore";
...
const SEARCH_CONFIG =
  report_CancelledOrdersEntity.operations.search;
...
function ReportFilters({
  ...
  const orderOptions = useDistinctOptions(
    rows,
    "orderID"
  );

  const productOptions = useDistinctOptions(
    rows,
    "productName"
  );
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="mt-0.5 text-[11px] text-[var(--color-ink-500,#78716a)]">
              اختر الطلبية أو المنتج أو الفترة الزمنية
            </div>
          </div>
        </div>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--color-ink-500,#78716a)]">
            الكميات المنتجة والهدر عبر مراحل الطلبيات الملغاة
          </p>
        </div>
...
function WasteAnalysis({ rows }) {
  ...
  return (
    <section className="border border-[var(--color-stone-300,#d8d2c6)] bg-white p-5">
      ...
      <p className="mt-1 text-xs text-[var(--color-ink-500,#78716a)]">
        توزيع الهدر في الطلبيات التي أُلغيت
      </p>
...
function CancelledOrdersExplorer({ rows }) {
  ...
  const sortedRows = useMemo(() => {
    return [...rows].sort((a, b) => {
      ...
      if (DATE_FIELDS.has(sort.field)) {
        const ad =
          new Date(av).getTime() || 0;
        const bd =
          new Date(bv).getTime() || 0;
  ...
  return (
    <section className="border border-[var(--color-stone-300,#d8d2c6)] bg-white">
      <div className="flex items-center justify-between border-b border-[var(--color-stone-200,#e7e2d8)] px-5 py-4">
        <div>
          <h2 className="font-display text-base font-bold text-[var(--color-ink-900,#2b2723)]">
            سجل الطلبيات الملغاة
          </h2>

          <p className="mt-1 text-xs text-[var(--color-ink-500,#78716a)]">
            تفاصيل الإنتاج والهدر لكل طلبية ملغاة
          </p>
        </div>
  ...
  return (
    <div className="border border-[var(--color-stone-300,#d8d2c6)] bg-white py-16 text-center">
      ...
      <p className="mt-4 text-sm text-[var(--color-ink-500,#78716a)]">
        جاري تحميل تقرير الطلبيات الملغاة…
      </p>
    ...
    <div className="flex flex-col items-center border border-[var(--color-stone-300,#d8d2c6)] bg-white px-6 py-20 text-center">
      ...
      <p className="mt-1 max-w-md text-xs leading-6 text-[var(--color-ink-500,#78716a)]">
        {hasFilters
          ...
          : "لا توجد بيانات متاحة للطلبيات الملغاة حاليًا."}
      </p>
...
export default function ReportCancelledOrdersPage() {
  const store =
    useReportCancelledOrdersStore();
  ...
  const rows = useMemo(() => {
    ...
    if (
      Array.isArray(searchState?.rows)
    ) {
  ...
  useEffect(() => {
    if (typeof search !== "function") {
      console.error(
        "ReportCancelledOrdersPage: search is not available"
      );
  ...
  function runSearch(nextFilters) {
    ...
    if (typeof search !== "function") {
      console.error(
        "ReportCancelledOrdersPage: search is not a function"
      );
          ...
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <h1 className="mt-2 font-display text-2xl font-bold text-[var(--color-ink-900,#2b2723)]">
                تقرير الطلبيات الملغاة
              </h1>

              <p className="mt-1 max-w-2xl text-sm text-[var(--color-ink-500,#78716a)]">
                تحليل الطلبيات الملغاة وما تم إنتاجه والهدر الناتج عنها عبر مراحل القص والتلوين والتعبئة.
              </p>
            </div>
