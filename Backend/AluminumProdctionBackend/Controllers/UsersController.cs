...
public class UsersController : ControllerBase
{
    ...
    // إضافة جديدة: تعديل جزئي - أرسل فقط الحقل الذي تريد تغييره
    // =====================================================

    [HasPermission(3)] // ⚠️ عدّل الرقم إذا صلاحية التعديل (UPDATE) عندكم مختلفة
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    [HttpPut("{id}/update-info", Name = "UpdateUserInfo")]
    public async Task<ActionResult> UpdateUserInfo(
        int id,
        [FromBody] UsersUpdateInfoDTO dto,
        CancellationToken cancellationToken)
    {
        dto.ID = id;

        var result =
            await UsersService.UpdateUserInfoAsync(
                dto,
                cancellationToken);
