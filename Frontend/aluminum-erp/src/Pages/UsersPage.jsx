...
// إضافة جديدة: حقول عملية تحديث بيانات المستخدم (UserName/FullName/IsActive)
const updateInfoFields = operations.updateInfo?.fields || [];
...
/*
|--------------------------------------------------------------------------
| إضافة جديدة: Modal تعديل بيانات المستخدم
| (UserName / FullName / IsActive) - كل حقل له checkbox خاص به،
| فقط الحقول المفعّلة تُرسل بالـ payload، الباقي يبقى كما هو
|--------------------------------------------------------------------------
*/
function UpdateUserInfoModal({
  ...
  const handleSubmit = (event) => {
    ...
    if (
      ...
      setError(
        "الرجاء تحديد حقل واحد على الأقل لتعديله.",
      );
    ...
    if (
      ...
      setError(
        "الاسم الكامل لا يمكن أن يكون فارغًا.",
      );
        ...
        >
          ...
          <div className="mt-5 flex justify-start gap-2">
            ...
            <button
              ...
              onClick={onClose}
              disabled={loading}
              className="rounded-md px-3.5 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 disabled:opacity-50"
            >
              إلغاء
            </button>
...
export default function UsersPage() {
  ...
  // إضافة جديدة: حالة Modal تعديل بيانات المستخدم + المستخدم المحدد + حالة التحميل
  ...
  */
  ...
  /*
  |--------------------------------------------------------------------------
  | إضافة جديدة: تعديل بيانات المستخدم (UserName / FullName / IsActive)
  | نداء مباشر عبر apiFetch لأن هذه العملية جديدة ولا تتعارض مع "update"
  | المحجوز أصلاً لتحديث كلمة السر داخل createEntityApi/createEntityStore
  |--------------------------------------------------------------------------
  */
...
const updateUserInfoRequest = async (payload) => {
  ...
  const url = `${API_URL}/${config.entity}/${payload.id}/update-info`;
  ...
  // دعم الحالتين:
  // 1. apiFetch يرجع Response
  // 2. apiFetch يرجع JSON جاهز
...
const handleUpdateUserInfo = async (payload) => {
  setUpdateInfoLoading(true);

  try {
    console.log("UPDATE USER INFO PAYLOAD:", payload);
  ...
  */
  ...
  return (
    ...
    >
                ...
                Array.from({
                  ...
                  (_, index) => (
                    ...
                    >
                      ...
                      {/* إضافة جديدة: خلية هيكلية لعمود الإجراءات */}
                ...
                rows.map(
                  (row, index) => (
                    ...
                    >
                      ...
                      {/* إضافة جديدة: زر تعديل بيانات المستخدم لكل صف */}
                      <td className="px-4 py-3">
                        {updateInfoFields.length > 0 && (
                          <button
                            type="button"
                            onClick={() => {
                              setSelectedUserForInfo(row);
      ...
      {/* =========================================================
          إضافة جديدة: Update User Info Modal
      ========================================================= */}

      {updateInfoOpen && selectedUserForInfo && (
        <UpdateUserInfoModal
