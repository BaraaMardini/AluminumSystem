...
public static class UsersService
{
    ...
    // =====================================================
    // UPDATE USER INFO (UserName / FullName / IsActive)
    // إضافة جديدة: المستخدم يحدد أي حقل يريد تعديله فقط
    // =====================================================

    public static async Task<ApiResult<object>> UpdateUserInfoAsync(
        UsersUpdateInfoDTO dto,
        CancellationToken cancellationToken = default)
    {
        ...
        if (!hasUserName && !hasFullName && !hasIsActive)
        {
            return new ApiResult<object>
            {
                Data = null,
                Message = "الرجاء تحديد حقل واحد على الأقل لتعديله.",
                ErrorType = ErrorType.InvalidId
            };
        }

        var result =
            await UsersData.UpdateUserInfoAsync(
                dto.ID,
                hasUserName ? dto.UserName : null,
                hasFullName ? dto.FullName : null,
                hasIsActive ? dto.IsActive : null,
                cancellationToken);
