...
export async function apiFetch(url, options = {}) {
  ...
  // استثناء مسار تسجيل الدخول: ما منطبّق منطق "انتهاء الجلسة" عليه أبدًا
  const isLoginRequest = url.includes("/LoginRequest/Login");

  // انتهاء صلاحية التوكن أو عدم صلاحيته → تسجيل خروج تلقائي
  if (response.status === 401 && !isLoginRequest) {
    clearToken();

    // متوافق مع HashRouter (#/login) بدل مسار مطلق
    // بيشتغل صح بالدفلوبمنت وبالنسخة المبنية (file://) الاثنين
