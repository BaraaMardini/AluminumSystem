...
2. Open `ConnectionString.cs` and update the connection string to point to your local SQL Server instance and the database created in step 1.
...
- Do not commit real database credentials or passwords to `ConnectionString.cs` or `appsettings.json` — use `appsettings.Development.json` (excluded via `.gitignore`) or environment variables for sensitive values.
